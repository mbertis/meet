...
defineFeature(feature, (test) => {
  test("When user hasn’t searched for a city, show upcoming events from all cities.", ({
    ...
    given("user hasn’t searched for any city", () => {
      // no code required as nothing has actually happened yet
    });
  ...
  test("User can select a city from the suggested list", ({
    ...
    given("user was typing “Berlin” in the city textbox", async () => {
      AppWrapper = await mount(<App />); //mount because we need to access a child - CitySearch
    ...
    and("the list of suggested cities is showing", () => {
      AppWrapper.update();
      expect(AppWrapper.find(".suggestions li")).toHaveLength(2); // two locations are Berlin and 'all'
    });
